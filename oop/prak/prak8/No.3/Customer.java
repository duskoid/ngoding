public class Customer {
  ...
  public void incrementOrderCount() throws OrderLimitExceededException {
    if (orderCount >= MAX_ORDERS_PER_CUSTOMER) throw new OrderLimitExceededException(String.format("Customer '%s' telah mencapai batas maksimum order (%d).", getName(), MAX_ORDERS_PER_CUSTOMER));
    orderCount++;
  }
  ...
  public void deductBalance(double amount) throws InvalidQuantityException, InsufficientBalanceException {
    ...
    if (balance < amount) throw new InsufficientBalanceException("Saldo tidak mencukupi. Saldo: Rp" + getBalance() + ", Dibutuhkan: Rp" + amount);
    balance -= amount;
  }
