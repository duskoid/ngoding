...
public class CheckoutService {
  ...
  public void checkout(String productId, int qty, String card) throws ProductNotFoundException, InsufficientStockException, PaymentFailedException {
    ...
    if (productId == null || productId.trim().isEmpty()) throw new InvalidRequestException("productId kosong");
    if (qty <= 0) throw new InvalidRequestException("qty harus > 0");
    if (card == null || card.trim().isEmpty()) throw new InvalidRequestException("card kosong");

    Product p = this.find(productId);
    if (p.getStock() < qty) throw new InsufficientStockException("Stock " + p.getStock() + " < " + qty);
    Boolean reserved = false; Boolean paid = false;
    p.reduce(qty);
    reserved = true;
    int amount = 100*qty;
    try {
