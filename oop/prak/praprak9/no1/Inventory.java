...
public class Inventory {
    ...
    public Item removeItem(String position) {
        ...
        if (grid.get(position) != null) {
            ...
            System.out.println(
                "Removed " + temp.getName() + " from position " + position
            );
            return temp;
        }
        return null;
    ...
    public Map<String, Item> filterByRarity(int minRarity) {
        ...
        for (Map.Entry<String, Item> e : grid.entrySet()) {
            if (e.getValue() != null) {
                if (e.getValue().getRarity() >= minRarity) {
                    output.put(e.getKey(), e.getValue());
                }
            }
        }
     ...
     */
    public Map<String, Item> filterByWeightRange(
        double minWeight,
        double maxWeight
    ) {
        Map<String, Item> output = new LinkedHashMap<>();
        ...
        for (Map.Entry<String, Item> e : grid.entrySet()) {
            if (e.getValue() != null) {
                if (
                    e.getValue().getWeight() <= maxWeight &&
                    e.getValue().getWeight() >= minWeight
                ) {
                    output.put(e.getKey(), e.getValue());
                }
            }
    ...
    public void displayGrid() {
        System.out.println("\n=== INVENTORY GRID ===");
        System.out.println(
            "Capacity: " + countItems() + "/" + (rows * cols) + " slots"
        );
        System.out.println(
            "Weight: " +
        ...
        for (int row = 0; row < rows; row++) {
            ...
            for (int col = 0; col < cols; col++) {
                ...
                if (item == null) {
                    ...
                    String display = item
                        .getName()
                        .substring(0, Math.min(12, item.getName().length()));
    ...
    public void displayDetailedInventory() {
        ...
        Collections.sort(
            items,
            new Comparator<Map.Entry<String, Item>>() {
                @Override
                public int compare(
                    Map.Entry<String, Item> e1,
                    Map.Entry<String, Item> e2
                ) {
    ...
    public void displayStatistics() {
        ...
        System.out.println(
            "Average Item Weight: " +
                String.format("%.1f", getAverageItemWeight()) +
                " kg"
        );
        System.out.println(
            "Average Rarity: " + String.format("%.2f", getAverageRarity()) + "*"
        );
